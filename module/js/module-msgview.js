...
IMApp.MessageView = (function($, _) {
  ...
  return {
    ...
    messageSendFaild: function(messagesId) {
      var messages = $('.message');

      messages.filter(function(index) {
        return $(this).attr('data-id') === messagesId;
      })
      .find('.warning').show();
    },
    resendMessage: function(messagesId) {
      $('.warning').hide();
    }
  };
}(jQuery, window._));
...
window.onMessageSendFaild = IMApp.MessageView.messageSendFaild;
window.resendMessage = IMApp.MessageView.resendMessage;
